...
static void ubus_send_request(ngx_http_request_t *r, json_object *obj, const char *sid, struct blob_attr *args)
{
	...
	ubus_invoke(ctx, du->obj, du->func, req.head, ubus_request_cb, du, 60000);
	if (ret) {
...
static void ngx_http_ubus_read_req(ngx_http_request_t *r)
{
    ...
    char *buffer = ngx_palloc(r->pool, cglcf->req_len);
...
static ngx_int_t ngx_http_ubus_elaborate_req(ngx_http_request_t *r)
{
	...
	case json_type_array:
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "Array detected");
        break;
	//	ubus_init_batch(cl);
	//	return ubus_next_batched_request(cl);
	default:
...
ngx_http_ubus_handler(ngx_http_request_t *r)
{
    ...
    //ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "Result: %s", cglcf->ubus.jsobj_end);
/*
    char *data = ngx_palloc(r->pool, body_data - 1 );
    int count = 0;
    ngx_chain_t *cl;
    ...
    ubus_data_send(r,cglcf,body_data,data);
...
/*
    ...
    */
    ...
    r->headers_out.content_length_n = sizeof(empty) - 1;
*/
    ...
    return ngx_http_output_filter(r, &cglcf->out_chain);
}
...
static ngx_int_t
ngx_http_ubus_init(ngx_http_ubus_loc_conf_t *cf)
{
