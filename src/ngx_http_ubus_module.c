...
static void free_output_chain(ngx_http_request_t *r,
    ngx_chain_t* chain);
...
static void parse_cors_from_header(ngx_http_request_t *r,
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        if (ngx_strcmp("origin", h[i].key.data)) {
            ...
            found_count++;
        } else if (ngx_strcmp("access-control-request-method",
                h[i].key.data)) {
...
static ngx_int_t ngx_http_ubus_send_header(
    ngx_http_request_t *r, ngx_http_ubus_loc_conf_t  *cglcf,
    ngx_int_t status, ngx_int_t post_len) {
...
static void setup_ubus_ctx_t(ubus_ctx_t *ctx,
    request_ctx_t *request, struct json_object *obj) {
...
static bool ubus_allowed(ubus_ctx_t *ctx, ngx_int_t script_timeout,
    ...
    uint32_t id;
    bool allow = false;
    struct blob_buf* req = ngx_pcalloc(ctx->request->r->pool,
        sizeof(struct blob_buf));
    ...
    ubus_invoke(ctx->request->ubus_ctx, id, "access", req->head,
...
static enum rpc_status ubus_send_request(request_ctx_t *request,
    ...
    struct blob_buf *req = ngx_pcalloc(request->r->pool,
        sizeof(struct blob_buf));
...
static enum rpc_status ubus_send_list(request_ctx_t *request,
   ubus_ctx_t *ctx, struct blob_attr *params) {
...
static enum rpc_status ubus_post_object(ubus_ctx_t *ctx) {
    ...
    if (!strcmp(data.method, "call")) {
        ...
        if (!cglcf->noauth && !ubus_allowed(ctx, cglcf->script_timeout,
            data.sid, data.object, data.function)) {
            err = ERROR_ACCESS;
            goto error;
        }
    ...
    while ( obj_done < len ) {
        ...
        for (concurrent = 0; concurrent < concurrent_thread ; concurrent++) {
            ...
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, request->r->connection->log, 0,
               "Spawning thread %d to process request %d",
               concurrent, obj_done);
            ...
            pthread_create(&threads[concurrent], NULL,
                (void *) ubus_post_object, ctx);
            threads_spawned++;
            obj_done++;
        }
...
ngx_http_ubus_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child) {
        ...
        ngx_conf_merge_uint_value(conf->script_timeout,
            prev->script_timeout, 60);
